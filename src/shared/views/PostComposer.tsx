...
interface AclPickerProps {
  ...
  onToggleXid: (xid: string, name: string) => void;
  onClearCustom: () => void;
}
...
const AclPicker: Component<AclPickerProps> = (props) => {
  ...
  const modeLabel: Record<AclMode, string> = {
    ...
    custom: `🤫 Custom${props.selectedXids.size > 0 ? ` (${props.selectedXids.size})` : ""}`,
  };
  ...
  return (
    <div class="relative shrink-0">
      ...
      <Show when={open() && props.mode === "custom"}>
        <div class="absolute bottom-full mb-2 left-0 z-50 w-72 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-xl overflow-hidden flex flex-col max-h-80">
          {/* Search */}
          ...
          {/* Selected chips */}
          <Show when={props.selectedXids.size > 0}>
            <div class="flex flex-wrap gap-1 px-3 py-2 border-b border-gray-100 dark:border-gray-800 shrink-0">
              <For each={[...props.selectedXids]}>
                {(xid) => {
                  const conn = (connections() ?? []).find((c) => c.xid === xid);
                  return (
                    <span class="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-300 dark:border-indigo-600 text-indigo-600 dark:text-indigo-300">
                      {conn?.name ?? xid.slice(0, 12) + "…"}
                      <button
                        type="button"
                        onClick={() => props.onToggleXid(xid, conn?.name ?? "")}
          ...
          <ul class="overflow-y-auto flex-1 py-1">
            ...
            <For each={filtered()}>
              {(c) => {
                const selected = () => props.selectedXids.has(c.xid);
                return (
                  <li>
                    <button
                      type="button"
                      onClick={() => props.onToggleXid(c.xid, c.name)}
                      class={
...
const PostComposer: Component<ComposerProps> = (props) => {
  ...
  // Set of contact xids for custom ACL
  const [allowXids, setAllowXids] = createSignal<Set<string>>(new Set());
  ...
  async function handleSubmit() {
    ...
    const mode = aclMode();
    if (mode === "public") {
      // No allow lists → Hubzilla treats it as public
      fd.append("contact_allow", "");
      fd.append("group_allow", "");
      fd.append("contact_deny", "");
      fd.append("group_deny", "");
    } else if (mode === "connections") {
      // Empty string signals "all connections" to Hubzilla
      fd.append("contact_allow", "");
      fd.append("group_allow", "");
      fd.append("contact_deny", "");
      fd.append("group_deny", "");
      fd.append("visibility", "1"); // connections-only flag
    } else {
      // Custom: append each selected xid as contact_allow[]
      const xids = [...allowXids()];
      if (xids.length === 0) {
        setError("Select at least one connection for a private post.");
        setSubmitting(false);
        return;
      }
      xids.forEach((xid) => fd.append("contact_allow[]", xid));
      fd.append("group_allow", "");
      fd.append("contact_deny", "");
      fd.append("group_deny", "");
    }
          ...
          >
            ...
            <footer class="flex flex-wrap items-center gap-2 px-3.5 py-2.5 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 shrink-0">
              ...
              <AclPicker
                ...
                selectedXids={allowXids()}
                onToggleXid={(xid) => toggleXid(xid)}
