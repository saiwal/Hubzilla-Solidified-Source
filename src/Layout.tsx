...
const Layout: ParentComponent = (props) => {
  ...
  const closeAll = () => { setRightOpen(false); setNavOpen(false); };
    ...
    <div class="fixed inset-0 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      <div class="flex h-full">
        ...
        <Show when={rightOpen() || navOpen()}>
          <div class="fixed inset-0 z-30 bg-black/40 lg:hidden" onClick={closeAll} />
        </Show>
        ...
        `}>
          <nav class="space-y-2">
            <For each={navItems()}>
              {(item) => <span onClick={closeAll}><NavItem {...item} /></span>}
            </For>
          </nav>
        </div>

        {/* Mobile Bottom Nav */}
        <nav class="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between h-16 px-6
                    bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 lg:hidden">
          <button onClick={() => { setNavOpen(o => !o); setRightOpen(false); }}
          ...
          <button onClick={() => { setRightOpen(o => !o); setNavOpen(false); }}
