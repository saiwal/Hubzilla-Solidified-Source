...
//   /photos/:nick                   → summary
//   /photos/:nick/album/:datum      → album
//   /photos/:nick/image/:datum      → image detail

export default function Photos() {
  // SolidJS router gives us whichever params exist for the matched route
  const params = useParams<{ nick?: string; datum?: string }>();

  // Infer view from current pathname — cleaner than threading a datatype param

  const location = useLocation();
  const datatype = () => {
    const p = location.pathname; // reactive — tracked by createEffect
