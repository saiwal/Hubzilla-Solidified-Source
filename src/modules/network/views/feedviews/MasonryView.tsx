...
function MasonryCard(props: { post: ThreadNode }) {
  ...
  return (
    <div class="break-inside-avoid mb-3 bg-white dark:bg-gray-800
      ...
      <div class="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700/50">
        <button onClick={() => handleLike(p.mid, p.iid!)}
        ...
        <button onClick={() => handleRepeat(p.mid, p.iid!)}
          class="flex items-center gap-1 text-xs transition-colors"
          classList={{
        ...
        <Show when={replyCount > 0}>
          <span class="ml-auto text-xs text-gray-400">{replyCount} {replyCount === 1 ? 'reply' : 'replies'}</span>
        </Show>
      </div>
    </div>
  );
